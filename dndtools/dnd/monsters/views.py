...
def monster_index(request):
    ...
    return render(request, 'dnd/monsters/monster_index.html', context={'monster_list': paginator.items(),
...
def monster_list_by_rulebook(request):
    ...
    return render(request, 'dnd/monsters/monster_list_by_rulebook.html', context=
...
def monsters_in_rulebook(request, rulebook_slug, rulebook_id):
    ...
    if not rulebook.slug == rulebook_slug:
        return permanent_redirect_view(request, 'dnd:monsters:monsters_in_rulebook',
                                       kwargs={
    ...
    return render(request, 'dnd/monsters/monsters_in_rulebook.html', context={
...
def monster_detail(request, rulebook_slug, rulebook_id, monster_slug, monster_id):
    ...
    return render(request, 'dnd/monsters/monster_detail.html', context={'monster': monster,
      'rulebook': monster.rulebook, 'monster_speeds': monster_speeds, 'monster_subtypes': monster_subtypes,
      'monster_skills': monster_skills, 'monster_feats': monster_feats, 'i_like_it_url': request.build_absolute_uri(),
      'inaccurate_url': request.build_absolute_uri(), 'display_3e_warning': is_3e_edition(monster.rulebook.dnd_edition),},)
